...
//设置加载时阴影显示,完成加载页面颜色回复正常
$.ajaxSetup({//ajax设置开始前完成后进行配置
    beforeSend(){//在。。。之前发送
        //开始前执行代码
        index = layer.load(1,{//加载
            shade:[0.1,"#000"]//阴影
        });
    },
    complete(){//结束后执行
...
$("#add-commit").on("click",function(){
    var paramStr = $("#add-form").serialize();//序列化，收集表单数据（在表单中）
    //name=hao&age=22&gender=man&tel=18236521755

    $.post("/index/add",paramStr)//参数一发送地址，参数二携带的数据
    .done(function(data){//data服务器返回的数据 。done发送成功
        //数据返回后应该做的操作
        //1.比如说，用户提示
        //2.显示数据
        //3.跳转
        ...
        if(!data.err){//如果没有错
            $("#add-form")[0].reset();//数据重置
            // $("#add-modal").modal("hide");//加载成功后关闭模态框
        }
    });
});
